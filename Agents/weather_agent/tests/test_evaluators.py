...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([], "Milpitas", "yes", False),
        (["Milpitas", "San Jose"], "Milpitas", "yes", False),
        ([], "", "no", True),
        (["Milpitas"], "", "no", False),
    ],
)
...
def test_evaluate_tool_execution(
    ...
    """Tool execution status should match the expected behavior."""
...
def test_evaluate_no_unhandled_exception(
    ...
    assert (
        evaluate_no_unhandled_exception(unhandled_exception)
        is expected_result
    )
...
def test_evaluate_response_presence(response, expected_result):
    """The agent should always provide a non-empty response."""

    assert evaluate_response_presence(response) is expected_result


@pytest.mark.parametrize(
    ("latency_seconds", "maximum_seconds", "expected_result"),
    [
        (2.5, 5.0, True),
        (5.0, 5.0, True),
        (5.1, 5.0, False),
...
def test_evaluate_latency(
    ...
    """Latency should pass when it is at or below the threshold."""
